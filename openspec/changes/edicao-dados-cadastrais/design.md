...
O DTO de edição só declara os campos da spec (celular pessoal, celular institucional, e-mail institucional, imagem, descrição, endereço). Campos fora da whitelist (CNPJ, razão social, e-mail pessoal, nome, nome fantasia, senha) são descartados automaticamente pelo `ValidationPipe`, sem gerar erro de validação — cobre a "ignorados" da spec sem escrever lógica própria.
Alternativa considerada: aceitar qualquer campo e ignorar manualmente os não editáveis no service — descartada por duplicar a whitelist em dois lugares (DTO e service).
